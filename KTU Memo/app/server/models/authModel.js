...
const authModel = {
    ...
    // Create new user
    createUser: async (name, email, phone, password, role) => {
        const hashedPassword = await bcrypt.hash(password, 10);
        const otp = generateOTP();
        const otpExpires = getOTPExpiry();

        const [result] = await pool.query(
            'INSERT INTO users (name, email, phone, password, role, otp, otp_expires_at) VALUES (?, ?, ?, ?, ?, ?, ?)',
            [name, email, phone, hashedPassword, role, otp, otpExpires]
        );
